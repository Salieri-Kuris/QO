...
def GP_LL_contri_finiteT(n,B,T):
    ...
    contri=0
    if (mu-energy[0])/T > 150:
        energy1=(mu-energy[0])/T-np.log(1+np.exp((mu+2*m*pow(v,2))/T))
        energy2=np.log(1+np.exp((mu-energy[1])/T))-np.log(1+np.exp((mu+2*m*pow(v,2))/T))
        energy3=np.log(1+np.exp((mu-energy[2])/T))
        contri+=-T*(energy1+energy2+energy3)
    else:
        energy1=np.log(1+np.exp((mu-energy[0])/T))-np.log(1+np.exp((mu+2*m*pow(v,2))/T))
        energy2=np.log(1+np.exp((mu-energy[1])/T))-np.log(1+np.exp((mu+2*m*pow(v,2))/T))
        energy3=np.log(1+np.exp((mu-energy[2])/T))
        contri+=-T*(energy1+energy2+energy3)
    return contri
...
for n in range(0,Ncutoff):
    ...
    if (mu-energy[0])/T > 150:
        ...
        energy2[n]=np.log(1+np.exp((mu-energy[1])/T))-np.log(1+np.exp((mu+2*m*pow(v,2))/T))
        energy3[n]=np.log(1+np.exp((mu-energy[2])/T))
        Phi_finiteT+=-T*(energy1[n]+energy2[n]+energy3[n])
