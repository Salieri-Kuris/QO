...
def GrandPotential(B,mu,T):
    ...
    for n in range(0,Ncutoff):
        energy=Energy(n,B)
        energy1[n]=np.logaddexp((mu-energy[0])/T, -(mu+2*m*pow(v,2))/T)
        energy2[n]=np.logaddexp((mu-energy[1])/T, -(mu+2*m*pow(v,2))/T)
        energy3[n]=np.logaddexp((mu-energy[2])/T, 0)
...
magnetic_field_inverse_range = np.linspace(1, 1.2, 50)
T_range = np.linspace(0.2,0.6 , 20)
